/**
 * Social-first roster-announcement directions for the Bell Pepper Open (Stories, 1080x1920).
 *
 * Optimized for DISTRIBUTION, not ops: the card exists to get featured players to repost it.
 * So names + photo + event facts (name/date/location) + CTA — no seed/status/format noise.
 *
 *   node scripts/story-assets/teams/render-concepts.mjs
 *
 * Output: concepts/social-1-werein.png, social-2-challenge.png
 */
...
const TEAM = { players: ['Colin Merk', 'Ryan Merk', 'Dave Wieczorek'] }
...
// Shared photo grade — visible up top, darkened where text sits.
...
/* ───────────────  SOCIAL 1 — "WE'RE IN"  (repost-bait, photo-led)  ─────────────── */
function werein(e, t) {
  ...
  .roster .nm{font-family:'Bebas Neue',sans-serif;font-size:90px;line-height:0.92;letter-spacing:0.01em;white-space:nowrap}
  ...
  .cta .h{margin-left:auto;font-size:22px;letter-spacing:0.14em;text-transform:uppercase;color:#f5f5f0;font-weight:700}
    ...
    <div class="frame">
      ...
      <div class="stack">
        ...
        <ul class="roster">${t.players.map(p => `<li><span class="tick"></span><span class="nm">${p}</span></li>`).join('')}</ul>
...
/* ───────────────  SOCIAL 2 — "CHALLENGE"  (comment/tag-bait)  ─────────────── */
function challenge(e, t) {
  return `<!DOCTYPE html><html><head><meta charset="utf-8"><style>${FONTS}${RESET}
  body{position:relative;font-family:'Space Mono',monospace;color:#f5f5f0}
  ${PHOTO}
  ...
  .roster .nm{font-family:'Bebas Neue',sans-serif;font-size:96px;line-height:1.0;letter-spacing:0.01em;white-space:nowrap}
  ...
  .meta{display:flex;align-items:center;gap:16px;margin-top:26px;font-size:22px;letter-spacing:0.12em;text-transform:uppercase;color:rgba(245,245,240,0.72)}
  .meta .b{font-family:'Bebas Neue',sans-serif;font-size:40px;letter-spacing:0.04em;color:#f5f5f0;text-transform:none}
  .meta .h{margin-left:auto;color:#f5f5f0;font-weight:700}
  .when{font-family:'Bebas Neue',sans-serif;font-size:46px;letter-spacing:0.04em;color:#4ade80;margin-top:6px}
  </style></head><body>
    <img class="photo" src="${heroA}"><div class="tint"></div><div class="grade"></div><div class="vignette"></div><div class="grain"></div>
    <div class="frame">
      <div class="top"><span>${e.handle}</span><span class="stamp">&rsaquo; ${e.dateStamp}</span></div>
      ...
      <div class="stack">
        ...
        <ul class="roster">${t.players.map(p => `<li class="nm">${p}</li>`).join('')}</ul>
...
const concepts = [
  { name: 'social-1-werein', html: werein(EVENT, TEAM) },
  { name: 'social-2-challenge', html: challenge(EVENT, TEAM) },
]

const browser = await chromium.launch()
for (const c of concepts) {
  const tmp = join(__dirname, `concepts/_tmp-${c.name}.html`)
  writeFileSync(tmp, c.html)
  const page = await browser.newPage({ viewport: { width: 1080, height: 1920 }, deviceScaleFactor: 1 })
  await page.goto(pathToFileURL(tmp).href, { waitUntil: 'networkidle' })
  await page.evaluate(() => document.fonts.ready)
  await page.waitForTimeout(500)
  await page.screenshot({ path: join(__dirname, `concepts/${c.name}.png`), clip: { x: 0, y: 0, width: 1080, height: 1920 } })
  console.log(`✓ ${c.name}.png`)
  await page.close()
  rmSync(tmp)
}
await browser.close()
console.log('\nDone. Comps in scripts/story-assets/teams/concepts/')
